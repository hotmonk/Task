  ...
  Item.findById(item_id)
    ...
    .exec(function(err1, res1) {
      if (err1) {
        ...
        if (
          itembid.interested_vendor_id.length === 0 &&
          itembid.counter === itembid.vendor_id.length
        ) {
          res.json({
            status: "fail",
          ...
          res.json({
            status: "fail",
            msg:
              "we will contact you as soon as we find more interested vendors"
          });
        } else {
              ...
              vendors = quotes.map(eachVendor => {
                return {
                  ...
                  distance: distance(
                    res2.latitude,
                    res2.longitude,
                    eachVendor.vendor_id.latitude,
                    eachVendor.vendor_id.longitude
                  )
                };
              });
              res.json(vendors);
  ...
  Item.findById(item_id)
    ...
    .exec(function(err1, res1) {
      if (err1) {
        ...
        newitembid.interested_vendor_id = newitembid.interested_vendor_id.filter(
          interested_vendor => {
                      ...
                      if (err3) {
                        ...
                        Vendor.findById(res20.vendor_id, function(err4, res4) {
                          ...
                          res4
                            .save()
                            .then(res5 => {
                              itembid.interested_vendor_id = itembid.interested_vendor_id.filter(
                                id => {
                              ...
                              itembid.save(function(err6, res6) {
                                if (err6) {
                                  ...
                                  Quote.findByIdAndDelete(quote_id, function(
                                    err7
                                  ) {
                                    if (err7) {
                                      console.log(err7);
...
router.post("/signUp", function(req, res) {
  ...
  Seller.findOne({ email }).then(seller => {
    if (seller)
      return res
        .status(400)
        .json({ msg: "Seller with the given email already exists" });
      ...
      if (err1) {
        ...
        res1.save(function(err2, res2) {
          if (err2) {
            ...
            vendor.defaulterAmount =
              (res1.transaction_id.quantity_taken *
                res1.transaction_id.price *
                config.get("commissionPercentage")) /
              100;
            vendor.save(function(err3, res3) {
              if (err3) {
                console.log(err3);
              } else {
                var quote_ids = res1.item_bid.interested_vendor_id.map(
                  quote => {
                    return quote._id;
                  }
                );
                var vendor_ids = res1.item_bid.interested_vendor_id.map(
                  vendor => {
...
router.post(
  "/:id/items",
  [sellerAuth, uploadImage.single("imageFile")],
  function(req, res) {
      ...
      .then(savedItem => {
        Seller.findById(req.params.id, function(err, response) {
          ...
          response
            .save()
            .then(seller => {
              Sub_cat.findById(req.body.sub_cat_id)
                .populate({
                    ...
                    arr = arr.filter(handle => {
                      return (
                        distance(
                          seller.latitude,
                          seller.longitude,
                          handle.vendor_id.latitude,
                          handle.vendor_id.longitude
                        ) < config.get("allowedRadius")
                      );
                    });
                    console.log(arr);
                    // if(arr.length===0){
                      ...
                      if (err3) {
                        ...
                        savedItem.save(function(err4, saveditem) {
                          if (err4) {
                            ...
                            arr = arr.map(item => {
                              News_feed.findById(
                                item.vendor_id.newsFeed,
                                function(err5, newsfeed) {
                                  if (err5) {
                                    console.log(err5);
                                  } else {
                            ...
                            res.status(200).json({
                              msg:
                                "your item is added for sale and a vendor will soon contact you"
                            });
                          }
                        });
                      }
