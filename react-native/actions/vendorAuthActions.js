...
export const loadVendor = () => (dispatch, getState) => {
  ...
  axios
    .get('http://192.168.43.199:4000/vendor/login/vendor', tokenConfig(getState))
    .then(res =>
      dispatch({
        type: VENDOR_LOADED,
        payload: res.data
...
export const signupVendor = ({ name, email, contact, address, password }) => dispatch => {
  ...
  axios
    .post('http://192.168.43.199:4000/vendor/signUp', body, config)
    .then(res =>
      dispatch({
...
export const vendorLogin = ({ email, password }) => dispatch => {
  ...
  axios
    .post('http://192.168.43.199:4000/vendor/login', body, config)
    .then(res =>
      dispatch({
