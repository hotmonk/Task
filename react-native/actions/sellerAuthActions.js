...
export const loadSeller = () => (dispatch, getState) => {
  ...
  axios
    .get('http://192.168.43.199:4000/seller/login/seller', tokenConfig(getState))
    .then(res =>
      dispatch({
        type: SELLER_LOADED,
        payload: res.data
...
export const signupSeller = ({ name, email, contact, address, password }) => dispatch => {
  ...
  await axios
    .post('http://192.168.43.199:4000/seller/signUp', body, config)
    .then(res =>
      console.log(res),
      dispatch({
...
export const sellerLogin = ({ email, password }) => dispatch => {
  ...
  axios
    .post('http://192.168.43.199:4000/seller/login', body, config)
    .then(res =>
      dispatch({
